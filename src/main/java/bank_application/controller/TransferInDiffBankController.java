...
@WebServlet("/TransferInDiffBank")
public class TransferInDiffBankController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (bankDao.findBank(bankName) != null) {
				...
				if (user != null) {
					...
					if (balance > amount && amount > 0) {
						...
						if (user1 != null) {
							...
							req.getRequestDispatcher("TransferInDiffBank.jsp").forward(req, resp);
						} else {
			...
			System.out.println("Receiver's bank does not exist");
			req.setAttribute("transfer", "Transfer success");
			req.getRequestDispatcher("TransferInDiffBank.jsp").forward(req, resp);
		}
	}
